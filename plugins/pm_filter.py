...
async def give_filter(client, message):
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if total_results == 0:
            ...
            return await message.reply_text(f"<b>🫡 ʜᴇʏ {message.from_user.mention} ,\n\n📭 ʜᴇʀᴇ ɪs ᴡʜᴀᴛ ɪ ғᴏᴜɴᴅ ɪɴ ᴍʏ ᴅᴀᴛᴀʙᴀsᴇ ғᴏʀ ʏᴏᴜʀ ǫᴜᴇʀʏ {search}.\n\n⚠️ ᴀꜰᴛᴇʀ 𝟷𝟶 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ.</b>")
...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         text=f"<b>ʜᴇʏ {user} 😍\n\n▪️ᴜsᴇ ɪɴʟɪɴᴇ sᴇᴀʀᴄʜ\n<u>ᴇxᴀᴍᴘʟᴇ</u>:\n@TIC_Auto_Filter_Bot ᴠɪᴋʀᴀɴᴛ ʀᴏɴᴀ\n@TIC_Auto_Filter_Bot ʙʀᴇᴀᴋɪɴɢ ʙᴀᴅ s𝟶𝟷\n\nᴏʀ\n\n▪️ᴄʀᴇᴀᴛᴇ ʏᴏᴜʀ ᴏᴡɴ ɢʀᴏᴜᴘ ᴀɴᴅ ᴀᴅᴅ ᴍᴇ ᴛʜᴇʀᴇ ᴀs ᴀᴅᴍɪɴ ᴛᴏ ɢᴇᴛ ғɪʟᴇs</b>")
...
async def next_page(bot, query):
    ...
    #     ]
        ...
        btn.insert(0, [
            InlineKeyboardButton("⚡ ᴄʜᴇᴄᴋ ʙᴏᴛ ᴘᴍ ⚡", url=f"https://t.me/{temp.U_NAME}")
        ])
    else:
        btn = []
    try:
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(0,[
            InlineKeyboardButton(text="ɴᴏᴛᴇ : ᴛʀʏ ʙᴏᴛʜ ᴏᴘᴛɪᴏɴs", callback_data="ident")
        ])
    
    btn.insert(1,[
        InlineKeyboardButton(text="⇊ ꜱᴇʟᴇᴄᴛ ʏᴏᴜʀ ʟᴀɴɢᴜᴀɢᴇ ⇊", callback_data="ident")
    ])
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    search = search.replace("_", " ")
    sea = ""
    season_search = ["s01","s02", "s03", "s04", "s05", "s06", "s07", "s08", "s09", "s1","s2", "s3", "s4", "s5", "s6", "s7", "s8", "s9", "s10", "season 01","season 02","season 03","season 04","season 05","season 06","season 07","season 08","season 09","season 10", "season 1","season 2","season 3","season 4","season 5","season 6","season 7","season 8","season 9"]
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("❆ ᴊᴏɪɴ ᴏᴜʀ ᴍᴀɪɴ ᴄʜᴀɴɴᴇʟ ❆", show_alert=True)
            return
        ...
        files, n_offset, total = await get_search_results(query.message.chat.id, search, offset=int(offset), filter=True)
        await send_all(client, query.from_user.id, files, ident, query.message.chat.id, query.from_user.first_name, query)
        await query.answer(f"Hey {query.from_user.first_name}, ᴀʟʟ ғɪʟᴇs ᴏɴ ᴛʜɪs ᴘᴀɢᴇ ʜᴀs ʙᴇᴇɴ sᴇɴᴛ sᴜᴄᴄᴇssғᴜʟʟʏ ᴛᴏ ʏᴏᴜʀ ᴘᴍ !", show_alert=True)
                
    elif query.data.startswith("killfilesdq"):
        ident, keyword = query.data.split("#")
        #await query.message.edit_text(f"<b>Fetching Files for your query {keyword} on DB... Please wait...</b>")
        files, total = await get_bad_files(keyword)
        await query.message.edit_text("<b>ғɪʟᴇ ᴅᴇʟᴇᴛɪᴏɴ ᴘʀᴏᴄᴇss ᴡɪʟʟ sᴛᴀʀᴛ ɪɴ 𝟻 sᴇᴄᴏɴᴅs !</b>")
            ...
            try:
                for file in files:
                    ...
                    if result.deleted_count:
                        logger.info(f'ғɪʟᴇ ғᴏᴜɴᴅ ғᴏʀ ʏᴏᴜʀ ǫᴜᴇʀʏ {keyword}! sᴜᴄᴄᴇssғᴜʟʟʏ ᴅᴇʟᴇᴛᴇᴅ {file_name} ғʀᴏᴍ ᴅᴀᴛᴀʙᴀsᴇ.')
                    deleted += 1
                    if deleted % 20 == 0:
                        await query.message.edit_text(f"<b>ᴘʀᴏᴄᴇss sᴛᴀʀᴛᴇᴅ ғᴏʀ ᴅᴇʟᴇᴛɪɴɢ ғɪʟᴇs ғʀᴏᴍ ᴅʙ. sᴜᴄᴄᴇssғᴜʟʟʏ ᴅᴇʟᴇᴛᴇᴅ {str(deleted)} ғɪʟᴇs ғʀᴏᴍ ᴅʙ ғᴏʀ ʏᴏᴜʀ ǫᴜᴇʀʏ {keyword} !\n\nᴘʟᴇᴀsᴇ ᴡᴀɪᴛ...</b>")
            except Exception as e:
                logger.exception(e)
        ...
        btn2 = [[
                 InlineKeyboardButton('Jᴏɪɴ Cʜᴀɴɴᴇʟ', url="https://t.me/TheInsomniacsClub"),
                 InlineKeyboardButton("Vɪᴇᴡ Sᴛᴀᴛᴜs", url=f"{query.message.link}")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/TheInsomniacsClub")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        buttons = [[
                    InlineKeyboardButton('✇ ʜᴏᴡ ᴛᴏ ᴜsᴇ ᴛʜᴇ ʙᴏᴛ ✇', callback_data='users')
                ],[
        ...
        buttons = [[
            InlineKeyboardButton('⤬ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘ ⤬', url=f'https://t.me/TheInsomniacsClub/51')
        ],[
        ...
        if set_type == 'is_shortlink' and query.from_user.id not in ADMINS:
            return await query.answer(text=f"ʜᴇʏ {query.from_user.first_name}, ʏᴏᴜ ᴄᴀɴ'ᴛ ᴄʜᴀɴɢᴇ sʜᴏʀᴛʟɪɴᴋ sᴇᴛᴛɪɴɢs ғᴏʀ ʏᴏᴜʀ ɢʀᴏᴜᴘ ! !\n\nɪᴛ's ᴀɴ ᴀᴅᴍɪɴ ᴏɴʟʏ sᴇᴛᴛɪɴɢ!", show_alert=True)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        if not settings["button"]:
            cap+="<b>ʜᴇʀᴇ ɪs ᴡʜᴀᴛ ɪ ғᴏᴜɴᴅ ɪɴ ᴍʏ ᴅᴀᴛᴀʙᴀsᴇ ғᴏʀ ʏᴏᴜʀ ǫᴜᴇʀʏ {search}.\n\n⚠️ ᴀꜰᴛᴇʀ 𝟷𝟶 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ.</b>"
            for file in files:
